...
try {
  ...
  // ---- 厳格モード中はバックアップのインポートで迂回できない ----
  // インポートは settings を丸ごと上書きするため、ここが素通りだと
  // 「ルールが空の JSON を読み込む」だけで厳格モードを回避できてしまう
  await options.bringToFront();
  await options.evaluate(async () => {
    const { settings } = await chrome.storage.local.get("settings");
  ...
  await sleep(400);
  await clickXPath(options, "//nav//button[contains(., 'バックアップ')]");
  const backupState = await options.evaluate(() => {
    const buttons = [...document.querySelectorAll("button")];
    const importBtn = buttons.find((b) =>
      b.textContent.includes("ファイルを選択してインポート"),
    );
    const exportBtn = buttons.find((b) => b.textContent.trim() === "エクスポート");
    return {
      importDisabled: importBtn?.disabled === true,
      exportEnabled: exportBtn?.disabled === false,
      notice: document.body.innerText.includes("厳格モード中はこの設定を変更できません"),
    };
  });
  check("厳格モード中はインポートが無効になる", backupState.importDisabled);
  check("厳格モード中でもエクスポートはできる", backupState.exportEnabled);
  check("厳格モード中である旨が表示される", backupState.notice);
  ...
  await sleep(300);
  await clickXPath(options, "//nav//button[contains(., 'ブロックルール')]");
