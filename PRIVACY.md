...
- 外部サーバーへの送信（拡張機能のコードに `fetch` / `XMLHttpRequest` / `WebSocket` は
  一切含まれていません）
- アクセス解析・トラッキング・広告
...
analytics, no remote code, and no network requests of any kind are used. Site icons come
from Chrome's built-in favicon API, so no URLs are sent to any external service.
