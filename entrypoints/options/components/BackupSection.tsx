...
export const BackupSection = ({ settings }: { settings: Settings }) => {
  // インポートは settings を丸ごと上書きするので、厳格モード中に許すと
  // 「ルールが空のバックアップを読み込む」だけでロックを迂回できてしまう。
  ...
  const applyImport = async () => {
    ...
    // 確認ダイアログを開いている間に厳格モードが有効化された場合に備えて再確認する
    if ((await getSettings()).strictMode) {
      setPendingImport(null);
      setMessage({ ok: false, text: STR.strictLocked });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="インポート">
        ...
        />
        {locked && (
          <p className="mb-3 flex items-center gap-1.5 rounded-lg bg-accent-500/[0.08] px-4 py-3 text-sm font-medium text-accent-600 dark:text-accent-400">
            <Icon name="lock" size={14} />
            {STR.strictLocked}
          </p>
        )}
        <Button disabled={locked} onClick={() => fileInputRef.current?.click()}>
          ファイルを選択してインポート
        </Button>
        {message !== null && (
          <p
